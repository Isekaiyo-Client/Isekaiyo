...
**Milestone 002 — Shell hardening & launch boundary.** Semantic design-token theming (AMOLED black + sakura pink), settings schema v2 with migration, a real `launch_instance` command that refuses honestly with `runtime.unavailable`, a typed Minecraft version model, and an About page. Scope and acceptance: [Milestone 2](docs/roadmap/milestone-002-shell-hardening.md).

**Milestone 003 — Launcher engine & loader architecture (in progress).** The real pipeline now exists in `ikk-minecraft`: official version manifest (cached, offline-tolerant), metadata parsing for modern and legacy eras, rule-based artifact resolution, SHA-1-verified atomic downloads, natives extraction (zip-slip guarded), Java discovery/compatibility, launch-plan construction, process tracking — plus a `LoaderProvider` architecture with working Fabric/Quilt providers and an explicit not-implemented Forge boundary. Status matrix and manual acceptance steps: [Milestone 3](docs/roadmap/milestone-003-launcher-engine.md).
